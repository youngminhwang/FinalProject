...
//  -------------------------------------------------------------------------
	...
	$("form").on("submit",function() {		//<form>가 submit 될 경우, 유효성 검사를 진행
		...
		var inval_Arr = new Array(12).fill(0);	
		...
		if(chkEmailAuth === 0) { 	// 0 : 실제 동작, 1 : Test 용도
			...
			if(chkMemberByEmail === 0){
				
				alert("해당 이메일로 가입된 회원이 이미 존재합니다.");
...
// 	});
	...
	$("#email").on("input change", function(){
		...
		}else if( mailJ.test($("#email").val()) && flag === 0 ){
			console.debug("mailJ test 1 ")
			$("#msg_email_check").text("유효한 이메일 입니다."); 
			$("#msg_email_check").css("color", "green");
			$("#btn_send_email").attr("disabled", false);
		
		}else if( mailJ.test($("#email").val()) == false && flag === 0 ){
			console.debug("mailJ test 2 ")
			$("#msg_email_check").text("유효하지 않은 이메일 입니다."); 
			$("#msg_email_check").css("color", "red");
			$("#btn_send_email").attr("disabled", true);
		
		}else if( $("#email").val() != oldEmail ) {
			console.debug("oldEmail test")
...
<div class="container">
	...
	<div class="col-sm-6 col-md-offset-3"> 
		<form action="/member/join" method="post" role="form" id="userCheck" name="member"> 
